...
class BookingsController extends Controller
{
                                ...
                                for ($i = 0; $i < $days; $i++) {
                                    ...
                                    if ($booking) {
                                        ...
                                        $arrBooking = [
                                            ...
                                            'guest' => $objBooking->booker && $objBooking->booker->user ? $objBooking->booker->user->first_name . ' ' . $objBooking->booker->user->last_name : null,
                                            ...
                                            'booker' => $objBooking->booker && $objBooking->booker->user ? $objBooking->booker->user->first_name . ' ' . $objBooking->booker->user->last_name : null,
    ...
    public function edit(Request $request, Booking $booking)
    {
        ...
        if ($booking->bookingRooms) {
            ...
            foreach ($booking->bookingRooms as $room) {
                ...
                if (!array_key_exists('primary_room', $responseArray)) {
                    ...
                    $responseArray['primary_room'] = $room && $room->room  ? $room->room->name . ' ' . $room->room->room_number : null;
                }
            }
        }
        ...
        $responseArray['amount_to_pay_with_dot'] = number_format((($responseArray['accommodation_price'] + $responseArray['accessories_price'] + $responseArray['city_tax'] + $responseArray['vat']) - $booking->discount) - $booking->totalPaid, 2, '.', '');

        $responseArray['price'] = number_format($responseArray['price'], 2, ',', '.');
        $responseArray['total_price'] = $responseArray['total_price'];
